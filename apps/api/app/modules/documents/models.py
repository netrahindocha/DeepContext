...
class SourceElement(Base):
    ...
    raw_content_text: Mapped[str] = mapped_column(Text, nullable=False)
    status: Mapped[str] = mapped_column(
        String(50),
        nullable=False,
        default="completed",
...
class SourceElementSummary(Base):
    __tablename__ = "source_element_summaries"
    ...
    source_element_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("source_elements.id", ondelete="CASCADE"),
        nullable=False,
        unique=True,
        index=True,
    )
